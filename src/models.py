...
class Wave_Model_Builder_Karpe:
    ...
    def build(self) -> keras.models.Model:
        ...
        x = keras.layers.Reshape((self._wave_len_samples, 1))(x)

        x = STFT(
            input_shape=(self._wave_len_samples, 1),
            n_fft=self._n_fft,
            hop_length=self._wave_len_samples // (msg_shape[0] - 1),
            pad_begin=False,
            pad_end=True,
        ...
        x = Pow(self._spec_power)(x)  # type: ignore

        x = MagnitudeToDecibel()(x)

        x = keras.layers.Reshape(msg_shape[0:2])(x)
